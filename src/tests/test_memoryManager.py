...
class TestMemoryManager(TestCase):
    def test_green(self):
        # given
        question_dict = TestMemoryTrainer.get_test_question_dict()
        trainer = MemoryTrainer.from_question_dict(question_dict)

        # when
        trainer.upsert_record('1', 1, datetime(2019, 1, 1), StatusEnum.PASS)
        trainer.upsert_record('1', 2, datetime(2019, 1, 4), StatusEnum.NOT_STARTED)
        ...
        # then
        last1 = trainer.training_log['1']['records'][-1]
        last2 = trainer.training_log['1']['records'][-2]
        ...
        last1 = trainer.training_log['2']['records'][-1]
        last2 = trainer.training_log['2']['records'][-2]
